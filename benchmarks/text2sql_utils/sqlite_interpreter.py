...
def execute_sql(predicted_sql, ground_truth, db_path, show_num_rows=10):
    ...
    pred_md_table = (
        "| " + " | ".join([f"Column{i + 1}" for i in range(len(predicted_res[0]))]) + " |\n"
    )
    ...
    gt_md_table = (
        "| " + " | ".join([f"Column{i + 1}" for i in range(len(ground_truth_res[0]))]) + " |\n"
    )
...
def execute_model(predicted_sql, ground_truth, db_path, meta_time_out=30):
    ...
    try:
        ...
        res_dict: dict | None = func_timeout(
            meta_time_out, execute_sql, args=(predicted_sql, ground_truth, db_path)
        )

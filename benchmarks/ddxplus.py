...
class MedicalDiagnosisBench(Bench):
    ...
    def postprocess_generation(self, res: str, idx: int = -1) -> int:
        ...
        if number not in self.LABEL2TEXT:
            ...
            print(
                Fore.RED
                + f"Prediction {res} not found in the label set. Please check your output format."
                + Style.RESET_ALL
            )
        return number

    def process_results(
        self, prediction: str, label: str, return_details: bool = False, **kwargs
    ) -> bool | dict:
    ...
    def save_output(self, output_path):
        df = pd.DataFrame({
            "id": list(range(len(self.predictions))),

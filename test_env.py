...
if __name__ == "__main__":
    ...
    for task, cls_ in TASKS.items():
        ...
        bench = cls_()
        for time_step, row in enumerate(
            tqdm(
                bench.get_dataset(),
                dynamic_ncols=True,
                bar_format=f"{{l_bar}}{Fore.BLUE}{{bar}}{Fore.RESET}{{r_bar}}",
            )
        ):
            ...
            if task in ["sql_generation_public"]:
                ...
                pred = label
            pred_res = bench.process_results(pred, label, return_details=True, time_step=time_step)
